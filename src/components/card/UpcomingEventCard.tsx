...
interface UpcomingEventCardProps {
  data: UpcomingEventCard;
}

export const UpcomingEventCard: React.FC<UpcomingEventCardProps> = ({ data }) => {
  const { image, title, location, date, price } = data;
  return (
    <div className="bg-white rounded-xl shadow-xl border-1 p-4">
      <img
        src={image}
        alt={title}
        className="w-full h-40 min-h-[102px] rounded-xl"
      />
      <div className="mt-1">
        ...
        <div className="flex items-center mt-2 gap-1 text-orange-600 text-lg font-semibold">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            className="w-5 h-5"
